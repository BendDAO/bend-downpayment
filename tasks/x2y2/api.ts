...
export class APIClient {
  ...
  constructor(network: Network, apiKey?: string) {
    ...
    this.httpClient.interceptors.request.use(AxiosLogger.requestLogger, AxiosLogger.errorLogger);
    this.httpClient.interceptors.response.use(AxiosLogger.responseLogger, AxiosLogger.errorLogger);
  }
  ...
  async _handleResponse(resp: AxiosResponse<any, any>) {
    if (![200, 400, 429].includes(resp.status)) throw new Error(`${resp.status} bad response: ${resp.statusText}`);
    if (resp.status === 429) throw new Error("rate limit");
    const respData = JSON.parse(resp.data);
    if (!respData || !respData.success) {
      const errors = respData?.errors;
      const msg = errors && errors.length > 0 ? `Err_${errors[0].code}` : "bad response";
      throw new Error(msg);
    }
    return respData;
  }
  ...
  async getSellOrder(maker: string, tokenAddress: string, tokenId: string) {
    ...
    return data instanceof Array && data.length > 0 ? (data[0] as Order) : undefined;
  }
