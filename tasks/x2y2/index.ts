...
import { encodeItemData, randomSalt, signBuyOffer, signSellOrder } from "./utils";
import { X2Y2R1__factory } from "./contracts/X2Y2R1__factory";
...
export function ethersWallet(privateKey: string, network: Network): ethers.Wallet {
  const networkMeta = getNetworkMeta(network);
  const provider = new ethers.providers.StaticJsonRpcProvider(networkMeta.rpcUrl, networkMeta.id);
...
export async function list({
  ...
  const order: X2Y2Order = makeSellOrder(network, accountAddress, expirationTime, [{ price, data }]);
...
async function cancelOrder(
  ...
  const tx = await market.cancel(input.itemHashes, input.deadline, input.v, input.r, input.s, callOverrides);
  return tx;
}
...
export async function cancelList(
  ...
  const order: Order | undefined = await apiClient.getSellOrder(accountAddress, tokenAddress, tokenId);
...
export async function buy(
  ...
  const order: Order | undefined = await getSharedAPIClient(network).getSellOrder("", tokenAddress, tokenId);
...
export async function offer({
  ...
  const salt = randomSalt();
  const dataMask = [{ token: ethers.constants.AddressZero, tokenId: "0x" + "1".repeat(64) }];
  const dataTokenId = isCollection ? "0" : tokenId ?? "0";
  const itemData = encodeItemData([{ token: tokenAddress, tokenId: dataTokenId }]);
...
export async function acceptOffer(
  ...
  return await acceptOrder(network, signer, OP_COMPLETE_BUY_OFFER, orderId, "0x", "0", tokenId ?? "", callOverrides);
}
...
export async function lowerPrice({
  ...
  const list: Order | undefined = await apiClient.getSellOrder(accountAddress, tokenAddress, tokenId);
  ...
  const data = encodeItemData([{ token: tokenAddress, tokenId }]);
  const order: X2Y2Order = makeSellOrder(network, accountAddress, expirationTime ?? parseInt(list.end_at), [
    { price, data },
  ]);
  await signSellOrder(signer, order);
